...
hooks:
  after_create: |
    git clone --depth 1 https://github.com/hjgraca/symphony-todo-app .
    cp -r /Users/henrigra/Development/personal/ai_projects/symphony/todo-app/.claude .
    git add -A && git commit -m "add claude agent config" --allow-empty
  before_remove: |
...
- `land`: when ticket reaches `Merging`, follow the `land` skill flow to squash-merge the PR.
...
3. Route to the matching flow:
   ...
   - `Merging` -> on entry, follow the `land` skill flow; do not call `gh pr merge` directly.
...
5.  Ensure the workpad includes a compact environment stamp at the top as a code fence line:
    ...
    - Example: `devbox-01:/tmp/symphony_todo_workspaces/HJG-10@7bdde33bc`
...
11. Before moving to `In Review`, poll PR feedback and checks:
    ...
    - Re-open and refresh the workpad before state transition so `Tasks`, `Acceptance Criteria`, and `Validation` exactly match completed work.
...
5. When the issue is in `Merging`, follow the `land` skill flow to squash-merge the PR. Do not call `gh pr merge` directly.
...
4. Remove the existing `## Plan` comment from the issue.
5. Create a fresh branch from `origin/main`.
6. Start over from the normal kickoff flow:
   - If current issue state is `Todo`, move it to `In Progress`; otherwise keep the current state.
   - Create a new bootstrap `## Plan` comment.
